...
def build_sequence_prompt(system_messages, chat_history, input_message, skip_count):
    sections = []
    truncated_messages = [*system_messages, *chat_history[skip_count:], input_message]
    for message in truncated_messages:
        if message.role in prompt_templates:
            template = prompt_templates[message.role]
            section = template.format(Content=message.content)
