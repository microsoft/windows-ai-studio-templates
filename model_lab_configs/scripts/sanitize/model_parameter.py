...
class RuntimeOverwrite(BaseModel):
    ...
    def Check(self, oliveJson: Any):
        if self.pyEnvPath and not checkPath(self.pyEnvPath, oliveJson):
            return False
        return True
...
class Section(BaseModel):
    ...
    def Check(self, templates: Dict[str, Parameter], _file: str, sectionId: int, oliveJson: Any, modelList):
        if not self.name:
            return False
        ...
        for i, parameter in enumerate(self.parameters):
            if parameter.template:
                ...
                if isinstance(template, Parameter) and hasattr(template, 'template') and template.template not in templates:
                ...
                if isinstance(template, Parameter):
                    ...
                    if hasattr(template, 'template') and template.template and isinstance(template.template, str):
                        parameter.applyTemplate(templates[template.template])
                elif isinstance(template, str) and template in templates:
                    parameter.applyTemplate(templates[template])
            ...
            if parameter.path and Section.datasetPathPattern(parameter.path):
                ...
                if parameter.values and modelList:
...
class ModelParameter(BaseModelClass):
    ...
    evalRuntime: Optional[str] = None  # Changed to str to avoid forward reference
    ...
    executeRuntimeFeatures: Optional[List[str]] = None  # Changed to str
    evalRuntimeFeatures: Optional[List[str]] = None     # Changed to str
    ...
    def Check(self, templates: Dict[str, Parameter], oliveJson: Any, modelList):
        ...
        if self.isQNNLLM:
            ...
            self.executeRuntimeFeatures = [RuntimeFeatureEnum.AutoGptq.value]
            self.evalRuntimeFeatures = [RuntimeFeatureEnum.Nightly.value]
    ...
    def checkOliveFile(self, oliveJson: Any):
        ...
        try:
            ...
            printWarning("deepdiff not available, skipping olive file comparison")
            return
        import os
