...
class ModelParameter(BaseModelClass):
    ...
    runtime: Parameter
    runtimeInPasses: Optional[Parameter] = None
    ...
    def CheckPasses(self, oliveJson: Any):
        ...
        def addRuntimeInPasses(runtime: Parameter, path: str, values: List[Any]):
            if not runtime.path:
                ...
                if runtime.extraPaths is None:
                    runtime.extraPaths = []
                if runtime.extraValues is None:
                    runtime.extraValues = []
                runtime.extraPaths.append(path)
                for i in range(len(values)):
                    if i >= len(runtime.extraValues):
                        runtime.extraValues.append([])
                    runtime.extraValues[i].append(values[i])

        if openVINOOptimumConversion or openVINOQuantization or openVINOEncapsulation:
            self.runtimeInPasses = Parameter(
                autoGenerated=True,
                name="Runtime in passes",
                type=ParameterTypeEnum.Enum,
                readOnly=True,
            )
            if openVINOOptimumConversion:
                addRuntimeInPasses(
                    self.runtimeInPasses,
