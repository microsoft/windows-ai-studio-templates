...
from .constants import (
    ParameterTypeEnum, ParameterDisplayTypeEnum, ParameterCheckTypeEnum,
    ParameterActionTypeEnum, ParameterTagEnum
)
from .utils import checkPath, printError, open_ex, printProcess
...
class Parameter(BaseModel):
    ...
    def Check(self, isTemplate: bool, oliveJson: Any = None, modelList: Any = None):
        if isTemplate:
            if self.template:
                return False
            return True
                ...
                if self.values:
                    ...
                    if self.tags and (ParameterTagEnum.EvaluationDataset in self.tags or ParameterTagEnum.QuantizationDataset in self.tags):
                        ...
                        for i in range(len(self.values) - 1):
                            ...
                            if modelList and hasattr(modelList, 'DatasetSplit') and value_in_list not in modelList.DatasetSplit:
                                printError(f"Value {value_in_list} not in DatasetSplit for {self.path}")
                                return False
                            if modelList and hasattr(modelList, 'DatasetSubset') and value_in_list not in modelList.DatasetSubset:
                                # No error for this, just warning
                                from .utils import printWarning
