...
class DebugInfo(BaseModel):
    ...
    def setupUseX(self, oliveJson: Any):
        ...
        if (
            sum(bool(v) for v in [self.useModelBuilder, self.useOpenVINOConversion, self.useOpenVINOOptimumConversion])
            > 1
        ):
    ...
    def Check(self, templates: Dict[str, Parameter], oliveJson: Any, modelList: ModelList):
        ...
        if self.addAmdNpu and currentEp != EPNames.VitisAIExecutionProvider.value:
            ...
            if evaluatorName and self.addAmdNpu.inferenceSettings:
                ...
                metricsNum = len(
                    pydash.get(
                        oliveJson, f"{OlivePropertyNames.Evaluators}.{evaluatorName}.{OlivePropertyNames.Metrics}"
                    )
                )
    ...
    if os.path.exists(ipynbFile):
        ...
        for name, modelParameter in modelItems.items():
            ...
            if modelParameter.isLLM:
                ...
                len(modelParameter.runtime.values) == 1
...
def main():
    ...
    result = subprocess.run(
        ["git", "status", "--porcelain"], cwd=configDir, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True
    )
